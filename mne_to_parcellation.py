...
def main():
    ...
    for hemi in ['lh', 'rh']:
        create_parcel_vertex_map(pnum_subj, hemi, dspm_run_dir, mri_subj_dir, parcellation_type)


def create_parcel_vertex_map(pnum_subj, hemi, dspm_run_dir, mri_subj_dir, parcellation_type):
    """

    Parameters
    ----------
    pnum_subj : str
        FreeSurfer subject code.
    hemi : str
        lh/rh.
    dspm_run_dir : str
        path to dspm run directory.
    mri_subj_dir : str
        path to FreeSurfer standard MRI dir.
    parcellation_type : str
        Annotation label, eg 'Schaefer2018_400Parcels_7Networks_order'

    Returns
    -------
    None. Saves numpy parcel vertex map to dspm_run_dir.
